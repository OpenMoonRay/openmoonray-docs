...
The shading [State](https://github.com/dreamworksanimation/moonray/blob/release/lib/rendering/bvh/shading/State.h) provides the shade function with geometric data at the shading point.

### Related ISPC macros
The vectorized shade function (ShadeFuncv) implementation is routinely followed by a call to the `DEFINE_MATERIAL_SHADER` macro which is defined in [ShaderMacros.isph](https://github.com/dreamworksanimation/moonray/blob/release/lib/rendering/shading/ispc/ShaderMacros.isph)
