...
on [Practical Path Guiding for Efficient
Light-Transport
...
This feature should be used only when needed. Basic path tracing handles the vast majority of cases we encounter. When
path guiding is enabled, the basic cases will run more slowly and consume more memory.
...
- Sample clamping can lead to inconsistent lighting results, especially when compared to non-path guided results. Don't
  use sample clamping with path guiding.
...
caustic reflection on a fairly rough surface.
...
| Ring 1:19:55 / 417Mb with path guiding | Ring 1:29:52 518Mb without path guiding (note adaptive sampling failures in
this case) |
