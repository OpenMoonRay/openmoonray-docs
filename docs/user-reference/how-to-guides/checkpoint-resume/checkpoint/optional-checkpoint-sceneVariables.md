...
{% highlight lua %}["checkpoint_overwrite"] = <bool> -- true or false {% endhighlight %}

If _checkpoint_overwrite_ is set to false, then MoonRay outputs each checkpoint file as the following filename:

{% highlight plaintext %}<checkpointFileName> + "_" + <tileSampleNumber> + ".exr" {% endhighlight %}
...
{% highlight lua %}["checkpoint multi version file name"] = <multiVersionFilename> {% endhighlight %}

If the _checkpoint_multi_version_file_name_ value is not empty, the checkpoint file names are changed as follows:

{% highlight lua %}<multiVersionFileName> + "_" + <tileSampleNumber> + ".exr" {% endhighlight %}
...
{% highlight lua %}["checkpoint_bg_write"] = <bool> -- true or false. default is true {% endhighlight %}
...
{% highlight lua %}["checkpoint_max_bgcache"] = <n> {% endhighlight %}
...
{% highlight lua %}["checkpoint_start_sample"] = <int> {% endhighlight %}
...
{% highlight lua %}["checkpoint_sample_cap"] = <int> -- sub pixel sample limit {% endhighlight %}
...
{% highlight lua %}["checkpoint time cap"] = <float> -- minute {% endhighlight %}
